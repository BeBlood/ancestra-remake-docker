...
public class GameThread implements Runnable
{
	...
	private void guild_add_perco()
	{
		...
		if(_perso.get_curCarte().get_maxTeam0() < 3 || _perso.get_curCarte().get_maxTeam1() < 3)//La map ne posséde pas de "places"
		{
	...
	private void guild_promote(String packet)
	{
		...
		{
			int guildId = SQLManager.isPersoInGuild(guid);	//Récupére l'id de la guilde du personnage qui n'est pas dans la mémoire
		...
		if(_perso.get_isTradingWith() < 0)
		{
			switch(packet.charAt(3))
			{
				...
				case '+'://Mettre un objet en vente
					...
					Objet obj = World.getObjet(itmID);//Récupére l'item

					int rAmount = (int)(Math.pow(10,amount)/10);
					//S'il veut mettre plus de cette objet en vente que ce qu'il posséde
	...
	private void Basic_chatMessage(String packet)
	{
		...
		switch(packet.charAt(2))
		{
			...
			break;
			case 'é'://Canal Admin (Console?) IG : /m
				//TODO
			break;
			case '^':// Canal Incarnam
				msg = packet.split("\\|", 2)[1];
				...
				{
					x = (Ancestra.FLOOD_TIME - x)/1000;//Calculamos a diferenéa em segundos
