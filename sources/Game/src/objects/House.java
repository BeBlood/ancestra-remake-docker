...
public class House
{
	...
	public void HopIn(Personnage P)//Entrer dans la maison
	{
		...
		if(h == null) return;
		if(h.get_owner_id() == P.getAccID() || (P.get_guild() != null && P.get_guild().get_id() == h.get_guild_id() && canDo(Constants.H_GNOCODE)))//C'est sa maison ou méme guilde + droits entrer sans pass
		{
			OpenHouse(P, "-", true);
		}
	...
	public static void HouseAchat(Personnage P)//Acheter une maison
	{
		...
		if(h.get_owner_id() > 0)
		{
			...
			if(Seller != null)
			{
				SocketManager.GAME_SEND_MESSAGE(Seller.get_curPerso(), "Une maison vous appartenant é été vendue "+h.get_sale()+" kamas.", Ancestra.CONFIG_MOTD_COLOR);
	...
	public void parseIntToRight(int total)
	{
		...
		if(haveRight.size() > 0)	//Si les droits contiennent quelque chose -> Vidage (Méme si le TreeMap supprimerais les entrées doublon lors de l'ajout)
			haveRight.clear();
