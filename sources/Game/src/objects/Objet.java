...
public class Objet {
	...
	public void parseStringToStats(String strStats)
	{
		...
		for(String s : split)
		{
			try
			{
				...
				if((!stats[3].equals("") && (!stats[3].equals("0") || statID == Constants.STATS_PETS_DATE || statID == Constants.STATS_PETS_PDV || statID == Constants.STATS_PETS_POIDS || statID == Constants.STATS_PETS_EPO)))//Si le stats n'est pas vide et (n'est pas égale é 0 ou est de type familier)
				{
	...
	{
		if(getTemplate().getType() == Constants.ITEM_TYPE_PIERRE_AME)//Si c'est une pierre d'éme vide
	...
	public int getPoidOfActualItem()//Donne le poid de l'item actuel
	{
		...
		for(String s : splitted)
		{
			...
			else if(statID == 138 || statID == 666 || statID == 226 || statID == 220)//Domages %,Domage renvoyé,Piége %
			{
				multi = 2;
			}
