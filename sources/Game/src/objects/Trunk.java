...
public class Trunk {
	...
	public void HopIn(Personnage P)//Ouvrir coffre
	{
		...
		else if(P.get_guild() == null && h.canDo(Constants.C_OCANTOPEN))//si on compare par id éa bug (guild null)
		{
			SocketManager.GAME_SEND_MESSAGE(P, "Ce coffre ne peut étre ouvert que par les membres de la guilde !", Ancestra.CONFIG_MOTD_COLOR);
		return;
		}
		else if(t.get_owner_id() > 0)//Une personne autre le posséde, il faut le code pour rentrer
		{
	...
	public void addInTrunk(int guid, int qua, Personnage P)
	{
		...
		{
			SocketManager.GAME_SEND_MESSAGE(P, "Le nombre d'objets maximal de ce coffre é été atteint !", Ancestra.CONFIG_MOTD_COLOR);
			return;
		}
