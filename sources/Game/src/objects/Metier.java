	...
	public static class JobAction
	{
		...
		private void doFmCraft()
		{
			...
			else if(isStatsChanging > 0 && isElementChanging == 0)//Si changement de stats
			{
				...
				// ou échec total : la FM n'a pas réussi et les bonus de toutes les caractéristiques diminuent proportionnellement é la puissance é la puissance de la rune. Utiliser de grosses runes est donc risqué.
			...
			if(!success)//Si echec
			{
				...
				else if(!obj.parseStatsString().isEmpty())//Si l'item posséde déjé des stats
				{
			...
			}else
			{
				...
				else if(isStatsChanging > 0 && isElementChanging == 0)//Si on modifier les stats (rune)
				{
					...
					if(ViewActualStatsItem(obj, Integer.toHexString(stat)) == 1 || ViewActualStatsItem(obj, Integer.toHexString(stat)) == 2)//L'item posséde le stats négatif ou positif
					{
					...
					else//L'item ne posséde pas le stats.
					{
