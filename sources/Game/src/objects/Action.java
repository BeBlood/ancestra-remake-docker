		...
		switch(ID)
		{
			...
			case -1://Ouvrir banque
				...
				if(cost > 0)
				{
                    ...
                    if(kamasRemaining < 0)//Si le joueur n'a pas assez de kamas SUR LUI pour ouvrir la banque
                    {
                        ...
                        {
                        	perso.set_kamas(0);//On puise l'entiéreter des kamas du joueurs. Ankalike ?
			...
			case 6://Apprendre un métier
				try
				{
					...
					if(mID == 	2 || mID == 11 ||
					   ...
					   mID == 60 || mID == 65)
					{
						if(perso.getMetierByID(mID) != null)//Métier déjé appris
						{
						...
						}else//Si c'est < ou = é 2 on apprend
						{
			...
			case 12://Spawn d'un groupe de monstre
				try
				{
					...
					if(inArena && !World.isArenaMap(perso.get_curCarte().get_id()))return;	//Si la map du personnage n'est pas classé comme étant dans l'aréne

					PierreAme pierrePleine = (PierreAme)World.getObjet(itemID);

					String groupData = pierrePleine.parseGroupData();
					String condition = "MiS = "+perso.get_GUID();	//Condition pour que le groupe ne soit lanéable que par le personnage qui é utiliser l'objet
			...
			case 50://Traque
				...
				if(perso.get_traque().get_time() < System.currentTimeMillis() - 600000 || perso.get_traque().get_time() == 0)
				{
					...
					//On ajoute le nom du type é recherché
					/*
